...
import { ImagePlus, LoaderCircle, PenLine, Send, Trash2, X } from 'lucide-react';

interface StoryComposerFabProps {
  visible: boolean;
  canPublish: boolean;
  onUnavailable: () => void;
  onPublish: (body: string, image: File | null) => Promise<void>;
}
...
const StoryComposerFab: React.FC<StoryComposerFabProps> = ({ visible, canPublish, onUnavailable, onPublish }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setError('');
    try {
      await onPublish(draft, imageFile);
      setDraft('');
      setImageFile(null);
      setOpen(false);
    } catch (publishError) {
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className='fixed z-[181] flex h-[min(430px,calc(100vh-1.75rem))] w-[min(440px,calc(100vw-1.75rem))] flex-col overflow-hidden rounded-[30px] border border-white/75 bg-background/95 shadow-[0_30px_90px_-28px_rgba(15,36,60,0.62)] backdrop-blur-2xl'
          >
            <form onSubmit={submit} className='flex min-h-0 flex-1 flex-col'>
              ...
              <div className='min-h-0 flex-1 overflow-y-auto px-5 pb-4 sm:px-6'>
                ...
                {previewUrl ? (
                  ...
                  <button type='button' onClick={() => fileInputRef.current?.click()} className='mt-3 flex min-h-24 w-full items-center justify-center gap-2 rounded-[20px] border border-dashed border-brand-blue/35 bg-brand-blue/[0.045] text-sm font-bold text-brand-blue transition hover:border-brand-blue hover:bg-brand-blue/[0.08]'><ImagePlus size={20} /> Adicionar foto</button>
                )}
                <input ref={fileInputRef} type='file' accept='image/jpeg,image/png,image/webp' className='sr-only' aria-label='Escolher foto da história' onChange={(event) => setImageFile(event.target.files?.[0] ?? null)} />
