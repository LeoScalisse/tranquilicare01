...
interface ScrollExpandProps extends Omit<
...
const ScrollExpand = ({
  ...
  const applyProgress = useCallback((progress: number) => {
    ...
    const restingHeight = config.contentPreview
      ? restingWidth
      : compact
        ? Math.max(config.startHeight, 62)
        : config.startHeight;
    ...
    if (config.startShape === "circle") {
      ...
      const restingCircleRadius =
        Math.min(stageWidth, stageHeight) * (restingWidth / 200);
      const coveringCircleRadius = Math.hypot(stageWidth, stageHeight) / 2 + 2;
      const circleRadius =
        restingCircleRadius +
        (coveringCircleRadius - restingCircleRadius) * eased;
    ...
    if (surroundRef.current) {
      ...
      surroundRef.current.style.visibility =
        surroundExit >= 1 ? "hidden" : "visible";
      const surroundInteractive = surroundExit < 0.8;
      surroundRef.current.style.pointerEvents = surroundInteractive
        ? "auto"
        : "none";
      surroundRef.current.inert = !surroundInteractive;
      surroundRef.current.setAttribute(
        "aria-hidden",
        String(!surroundInteractive),
      );
    }
  ...
  useEffect(() => {
    ...
    const contentScroller =
      overlayChild instanceof HTMLElement ? overlayChild : null;
    ...
    const measure = () => {
      ...
      contentScrollDistance =
        useWindowScroll && config.contentPreview && contentScroller
      ...
      track.style.height = `${
        ...
        expansionDistance +
        contentScrollDistance +
        stageHeight * Math.max(0, config.holdDistance)
      }px`;
      const width = root.clientWidth || stageHeight;
      stage.style.setProperty(
        "--se-title-size",
        `${clamp(width * 0.065, 28, 72)}px`,
      );
    };
    ...
    const readProgress = () => {
      ...
      if (useWindowScroll) {
        return clamp(
          -track.getBoundingClientRect().top / expansionDistance,
          0,
          1,
        );
      }
    ...
    const syncContentScroll = () => {
      if (
        !useWindowScroll ||
        !propsRef.current.contentPreview ||
        !contentScroller
      )
        return;
    ...
    >
      <div ref={trackRef} className="scroll-expand__track">
        <div ref={stageRef} className="scroll-expand__stage">
          <div
            ref={ambientRef}
            className="scroll-expand__ambient"
            aria-hidden="true"
          />
          {surround ? (
            <div ref={surroundRef} className="scroll-expand__surround">
              {surround}
            </div>
          ) : null}
          <div ref={frameRef} className="scroll-expand__frame">
            ...
            {restingOverlay ? (
              <div
                ref={restingOverlayRef}
                className="scroll-expand__resting-overlay"
              >
                {restingOverlay}
              </div>
            ) : null}
