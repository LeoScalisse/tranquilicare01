  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 z-[2] p-5 text-white">
        <span
          className={`mb-2 inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${categoryTheme.bg} ${categoryTheme.text} ${categoryTheme.border}`}
        >
          {ngo.category}
        </span>
        ...
        <p className="mt-2 line-clamp-2 text-sm font-medium leading-5 text-white/85">
          {cardSummary}
        </p>
        {ngo.posts?.length > 0 && (
