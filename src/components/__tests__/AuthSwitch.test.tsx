...
vi.mock("@/lib/auth", () => ({
  ...
  canUseGoogle: false,
  defaultDestForAccount: (role: string) =>
    role === "ngo" ? "/ngo/profile" : "/donor/profile",
...
describe("AuthSwitch signup", () => {
  ...
  it.each(signupPaths)(
    ...
    async (path) => {
      ...
      await user.type(
        screen.getByPlaceholderText(path.namePlaceholder),
        path.name,
      );
      await user.type(
        screen.getByPlaceholderText(path.emailPlaceholder),
        path.email,
      );
      await user.type(
      ...
      fireEvent.submit(
        screen.getByPlaceholderText(path.emailPlaceholder).closest("form")!,
      );

      expect(await screen.findByText(/Código de verificação/i)).toBeTruthy();
      expect(
        screen
          .getAllByText(path.email)
  ...
  it("uses the folded expanding style for every verification digit", async () => {
    ...
    expect(
      codeInputs.every((input) =>
        input.classList.contains("verification-code-input"),
      ),
    ).toBe(true);
  ...
  it("waits before allowing another confirmation email request", async () => {
    ...
    expect((resendLabel.closest("button") as HTMLButtonElement).disabled).toBe(
      true,
    );
  });
  ...
  it("shows an actionable error for an invalid email before submitting", async () => {
    ...
    fireEvent.submit(
      screen.getByRole("button", { name: path.submitLabel }).closest("form")!,
    );
  ...
  it("keeps the donor journey ready after verifying the emailed code", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /Personalizar meu perfil/i }),
    );
    await waitFor(() => {
      expect(screen.getByTestId("location").textContent).toBe(
        "/donor/profile?setup=1",
      );
    });
  });
...
describe("SmoothInput", () => {
  ...
  it("keeps the animated caret enabled in authentication fields", async () => {
    ...
    const emailInput = screen.getByPlaceholderText(
      signupPaths[0].emailPlaceholder,
    );
    ...
    expect(
      screen.getByLabelText("Senha", { selector: "input" }).style.caretColor,
    ).toBe("transparent");
  });
  ...
  it("tracks a selection moved by browser autocomplete after focus", async () => {
    render(
      <SmoothInput
        value={"a".repeat(40)}
        onChange={() => undefined}
        aria-label="Campo com seleção tardia"
      />,
    );

    const input = screen.getByLabelText(
      "Campo com seleção tardia",
    ) as HTMLInputElement;
    const measure = input.parentElement?.querySelector<HTMLSpanElement>(
    ...
    measure!.getBoundingClientRect = () => ({
      ...
      y: 0,
      top: 0,
      right: (measure!.textContent?.length ?? 0) * 10,
      bottom: 20,
      left: 0,
      width: (measure!.textContent?.length ?? 0) * 10,
      height: 20,
      toJSON: () => undefined,
    });
  ...
  it("can keep the native caret for browser-managed autocomplete fields", () => {
    ...
    expect(
      screen.getByLabelText("E-mail com autocomplete").style.caretColor,
    ).toBe("");
  });
