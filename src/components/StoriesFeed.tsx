...
const StoriesFeed: React.FC<StoriesFeedProps> = ({ stories, onSelectNGO }) => {
  ...
  return (
    <div className="h-[calc(100vh-80px)] md:h-[calc(100vh-64px)] w-full bg-black relative overflow-hidden select-none">
      {/* Subtle ambient glow */}
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Pause Indicator */}
                {isCurrent && isPaused && story.type === 'video' && (
                ...
                {story.type === 'video' ? (
                  <video
                    ...
                    src={story.url}
                    className={`w-full h-full object-cover transition-opacity duration-700 ${isLoaded[story.id] ? 'opacity-100' : 'opacity-0'}`}
                    loop
                    playsInline
                    muted={!isCurrent}
                    preload="auto"
                    onCanPlay={() => handleMediaLoad(story.id)}
                    onLoadedData={() => handleMediaLoad(story.id)}
                    onTimeUpdate={isCurrent ? handleTimeUpdate : undefined}
                  />
                ) : (
                  <img
                    src={story.url}
                    alt={story.ngoName || 'Story'}
                    className={`w-full h-full object-cover transition-opacity duration-700 ${isLoaded[story.id] ? 'opacity-100' : 'opacity-0'}`}
                  ...
                  <div className="absolute bottom-6 left-4 right-4 text-white z-20">
                    ...
                    >
                      <div className="relative">
                        <img 
                          src={story.ngoImage} 
                          className="w-10 h-10 rounded-full border-2 border-brand-yellow shadow-lg bg-white object-cover" 
                          alt={story.ngoName}
                        />
                    ...
                    {story.caption && (
                      <p className="text-xs text-gray-100 mb-4 drop-shadow-md leading-relaxed line-clamp-2 bg-black/20 backdrop-blur-sm p-2 rounded-xl border border-white/5">
                        {story.caption}
                      </p>
                    )}
                ...
                {isCurrent && (
                  <button 
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col items-center gap-2 group z-20"
                  >
      ...
      <div className="pointer-events-none absolute bottom-6 left-1/2 -translate-x-1/2 z-50">
        <div className="flex items-center gap-2 rounded-full bg-white/5 px-4 py-2 text-xs text-white/50 backdrop-blur-sm border border-white/10">
      ...
      <div className="absolute left-4 top-1/2 -translate-y-1/2 z-50">
        <div className="flex flex-col items-center rounded-full bg-white/5 px-3 py-4 backdrop-blur-sm border border-white/10">
          <span className="text-lg font-bold text-white">
            {String(currentIndex + 1).padStart(2, '0')}
          </span>
          <div className="my-1 h-px w-4 bg-white/20" />
