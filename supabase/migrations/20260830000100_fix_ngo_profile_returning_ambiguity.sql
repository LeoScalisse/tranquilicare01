-- Correct the onboarding RPC installed by 20260829000100.
--
-- `RETURNS TABLE (user_id, is_founder)` creates PL/pgSQL variables with
-- those names. The previous INSERT ... RETURNING used unqualified column
-- names, which Postgres therefore considered ambiguous (SQLSTATE 42702).
-- Qualifying the target table aliases keeps the RPC compatible and does not
-- modify any organization data.
