...
const NGOAccountProfile: React.FC = () => {
  ...
  if (loading) return <div className='grid min-h-screen place-items-center bg-white'><Loader2 className='animate-spin text-brand-blue' size={36} /></div>;
      ...
      <AnimatePresence>
        {editing && (
          <motion.div className='fixed inset-0 z-[130] grid place-items-center bg-brand-ink/70 p-4 backdrop-blur-sm' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={(event) => event.target === event.currentTarget && setEditing(false)}>
            <motion.div initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.98 }} className='w-full max-w-md overflow-hidden rounded-lg bg-white shadow-2xl'>
              <div className='flex items-center justify-between bg-brand-ink px-5 py-4 text-white'><h2 className='font-display text-xl font-semibold'>Editar perfil</h2><button onClick={() => setEditing(false)} className='grid h-9 w-9 place-items-center rounded-full bg-white text-brand-blue' aria-label='Fechar'><X size={19} /></button></div>
              <div className='p-6'>
