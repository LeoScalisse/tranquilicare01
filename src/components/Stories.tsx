...
const Stories: React.FC<StoriesProps> = ({ onOpenNGO }) => {
  ...
  return (
    <div className='mx-auto max-w-6xl px-4 pb-32 pt-8 md:pt-11'>
      ...
      {stories.length ? <section className='mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3'>{stories.map((story, index) => <article key={story.id} className='overflow-hidden rounded-lg border-2 border-border bg-background'><button onClick={() => setActiveIndex(index)} className='group relative block aspect-[4/5] w-full overflow-hidden bg-secondary text-left'>{story.type === 'image' ? <img src={story.url} alt={story.caption || 'História de impacto'} className='h-full w-full object-cover transition-transform duration-500 group-hover:scale-105' /> : <><video src={story.url} className='h-full w-full object-cover' preload='metadata' /><span className='absolute inset-0 grid place-items-center bg-black/20'><Play size={38} className='fill-white text-white' /></span><Video size={20} className='absolute right-3 top-3 text-white' /></>}<span className='absolute inset-x-0 bottom-0 bg-gradient-to-t from-brand-ink/90 to-transparent p-4 pt-20 text-white'><span className='flex items-center gap-2'><img src={story.ngoImage} alt='' className='h-9 w-9 rounded-full border-2 border-white object-cover' /><span className='min-w-0'><span className='flex items-center gap-1 text-sm font-bold'>{story.ngoName}{story.verified && <ShieldCheck size={14} className='text-brand-yellow' />}</span><span className='line-clamp-2 text-xs text-white/75'>{story.caption || 'Nova atualização de impacto'}</span></span></span></span></button><button onClick={() => onOpenNGO(story.ngoId)} className='flex w-full items-center justify-between px-4 py-3 text-sm font-bold text-brand-blue'><span className='inline-flex items-center gap-2'><Heart size={16} />Conhecer a causa</span><ChevronRight size={17} /></button></article>)}</section> : <div className='mt-10 rounded-lg border-2 border-dashed border-border p-12 text-center text-muted-foreground'>As primeiras histórias aparecerão aqui.</div>}
      ...
      <AnimatePresence>
        {activeStory && <motion.div className='fixed inset-0 z-[130] grid place-items-center bg-brand-ink/95 p-3 backdrop-blur-md' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setActiveIndex(null)}>
          <div className='absolute left-4 right-4 top-4 flex gap-1'>{stories.map((story, index) => <span key={story.id} className={`h-1 flex-1 rounded-full ${index <= (activeIndex ?? 0) ? 'bg-background' : 'bg-background/25'}`} />)}</div>
          <button onClick={() => setActiveIndex(null)} className='absolute right-4 top-8 grid h-10 w-10 place-items-center rounded-full bg-background text-brand-blue' aria-label='Fechar história'><X size={19} /></button>
          {stories.length > 1 && <><button onClick={(event) => { event.stopPropagation(); move(-1); }} className='absolute left-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-background/15 text-white backdrop-blur md:left-8' aria-label='História anterior'><ChevronLeft /></button><button onClick={(event) => { event.stopPropagation(); move(1); }} className='absolute right-3 z-10 grid h-10 w-10 place-items-center rounded-full bg-background/15 text-white backdrop-blur md:right-8' aria-label='Próxima história'><ChevronRight /></button></>}
