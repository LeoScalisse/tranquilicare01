...
const StoryReportMenu: React.FC<StoryReportMenuProps> = ({ canReport, onRequireAuth, onReport }) => {
      ...
      <Popover.Portal>
        <Popover.Content sideOffset={8} align='end' collisionPadding={12} className='z-[175] w-[min(20rem,calc(100vw-1.5rem))] outline-none'>
          <AnimatePresence mode='wait' initial={false}>
            {!formVisible ? (
              <motion.div key='menu' role='menu' aria-label='Opções da história' initial={reduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.96, y: -5 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: reduceMotion ? 0.01 : 0.2 }} className='rounded-[18px] border border-brand-ink/10 bg-background p-1.5 shadow-[0_18px_55px_-24px_rgba(15,36,60,0.55)]'>
              ...
              <motion.form key='form' onSubmit={submit} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: reduceMotion ? 0.01 : 0.22 }} className='rounded-[22px] border border-brand-ink/10 bg-background p-4 shadow-[0_20px_60px_-24px_rgba(15,36,60,0.58)]'>
